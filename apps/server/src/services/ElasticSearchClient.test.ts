...
describe("elasticSearchClient", () => {
  ...
  describe("getRecommendedInstitutions", () => {
    it("returns an empty array if recommendedInstitutions is undefined", async () => {
      jest.spyOn(preferences, "getPreferences").mockResolvedValue({
        ...testPreferences,
        recommendedInstitutions: undefined,
      });

      expect(
        await getRecommendedInstitutions({
          jobTypes: [ComboJobTypes.TRANSACTIONS],
        }),
      ).toEqual([]);
    });

    it("returns an empty array if recommendedInstitutions is an empty array", async () => {
      jest.spyOn(preferences, "getPreferences").mockResolvedValue({
        ...testPreferences,
  ...
  describe("getUcpIdFromAggregatorInstitutionCode", () => {
    beforeEach(() => {
      ElasticSearchMock.clearAll();
    });

    it("returns the UCP ID when institution is found with the aggregator code", async () => {
      const aggregator = "mx";
      const institutionCode = "mx_bank_123";
      const expectedUcpId = "UCP-1234";
      ...
      ElasticSearchMock.add(
        {
          ...
          body: {
            query: {
              term: {
                [`${aggregator}.id.keyword`]: institutionCode,
              },
            },
            size: 1,
          },
      ...
      const result = await getUcpIdFromAggregatorInstitutionCode(
        aggregator,
        institutionCode,
      );

      expect(result).toBe(expectedUcpId);
    });

    it("returns null when no institution is found with the aggregator code", async () => {
      const aggregator = "mx";
      const institutionCode = "non_existent_code";
    ...
    it("handles test aggregator IDs by converting them to non-sandbox versions", async () => {
      const testAggregator = "mx_int";
      const institutionCode = "mx_bank_123";
      const expectedUcpId = "UCP-5678";
    ...
    it("works with different aggregator types", async () => {
      const aggregator = "plaid";
      const institutionCode = "plaid_ins_123";
      const expectedUcpId = "UCP-9999";
