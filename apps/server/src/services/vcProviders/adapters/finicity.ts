...
// import { parse as parseAddress } from '@universe/address-parser'
...
export function mapIdentity(customerId: string, accountOwnerDetail: { addresses: any, ownerName: string, OwnerName: any, emails: Array<{ email: string }>, phone: Array<{ type: any, country: any, phone: any }> }) {
  const addresses = []
  for (const add of accountOwnerDetail.addresses) {
    try {
      // TODO: fix address parsing
      // const parsedAddress = parseAddress(add.ownerAddress)
      // addresses.push({
      //   line1: [parsedAddress.streetPreDir, parsedAddress.streetName, parsedAddress.streetType, parsedAddress.streetPostDir].filter(p => p).join(' '),
