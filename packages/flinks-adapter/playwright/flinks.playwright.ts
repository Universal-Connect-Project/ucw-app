...
import { FLINKS_PROFILES_A_UCP_INSTITUTION_ID } from "../src/testInstitutions";
import { FLINKS_AGGREGATOR_STRING } from "../src";
...
const makeAConnection = async (
  ...
  expect(obj.metadata.aggregator).toEqual("flinks_sandbox");
  ...
  userId: [
    async ({ request }, use) => {
      ...
      await request.delete(
        `http://localhost:8080/api/aggregator/flinks_sandbox/user/${userId}`,
      );
    },
    { scope: "test" },
  ],
...
test.describe("Flinks Adapter Tests", () => {
  ...
  test(`makes a connection with ${ComboJobTypes.TRANSACTION_HISTORY}, records a successful event, and returns transactions`, async ({
    ...
    const urlToIntercept = `http://localhost:8080/institutions/${FLINKS_PROFILES_A_UCP_INSTITUTION_ID}`;
    ...
    const performanceEvent = await expectPerformanceEvent({
      ...
      institutionId: FLINKS_PROFILES_A_UCP_INSTITUTION_ID,
      aggregatorId: FLINKS_AGGREGATOR_STRING,
    });
