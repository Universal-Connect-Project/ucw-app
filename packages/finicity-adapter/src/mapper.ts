...
export function mapTransaction(t: any, accountId: string){
  // Deposit Transaction
...
export function mapIdentity(customerId: string, accountOwnerDetail: any){
  const addresses = [];
  ...
  const ret = {
    ...
    addresses,
    email: accountOwnerDetail.emails?.map((e: any) => e.email),
    telephones: accountOwnerDetail.phone?.map((p: any) => ({
      type: p.type,
      country: p.country,
      number: p.phone
    })),
...
export function mapFiAttributes(fi: any){
  return [
    {
      name: 'name',
      value: fi.name
