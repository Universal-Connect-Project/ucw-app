...
export default class AkoyaClient {
  ...
  getOauthUrl(institutionId: string, state: string) {
    return `${this.apiConfig.basePath}/auth?connector=${institutionId}&client_id=${this.apiConfig.clientId}&redirect_uri=${this.client_redirect_url}&state=${state}&response_type=code&scope=openid profile offline_access`;
  }
